...
int CompareStrsCI(const void *left, const void *right); // CI : 大小写不敏感 case insensitive
...
int main(void)
{
  ...
  // char **name_ptr = bsearch(&key_name, names, sizeof names / sizeof *names, sizeof *names, CompareStrs);

  // char **name_ptr = bsearch(&key_name, names, sizeof names / sizeof *names, sizeof *names, CompareStrsAddress);
  ...
  char **name_ci_ptr = bsearch(&key_name, names, sizeof names / sizeof *names, sizeof *names, GetCompareFunction(1));
